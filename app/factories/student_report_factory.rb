class StudentReportFactory
  ...
  def generate
    ...
    pdf.text t_enum(student, :situation) || "-", size: 11, align: :center
    pdf.move_down 120
    ...
    render_section(pdf, "Ocorrências", %w[Data Título Descrição Status Gravidade Tipo], student.occurrencies.map { |o|
      [o.created_at.strftime("%d/%m/%Y"), o.title, o.description.truncate(50), t_enum(o, :status), t_enum(o, :severity), t_enum(o, :kind)]
    })
  
    render_section(pdf, "Orientações", %w[Data Título Área Status], student.orientations.map { |o|
      [o.created_at.strftime("%d/%m/%Y"), o.title, t_enum(o, :area), t_enum(o, :status)]
    })
  
    render_section(pdf, "Atendimentos", %w[Data Assunto Área Status], student.schedules.map { |s|
      [s.starts_at.strftime("%d/%m/%Y %H:%M"), s.subject, t_enum(s, :area), t_enum(s, :status)]
    })
  
    render_section(pdf, "Autorizações", %w[Data Descrição Status], student.authorizations.map { |a|
      [a.date.strftime("%d/%m/%Y"), a.description, t_enum(a, :status)]
    })
  
    render_section(pdf, "Condições Especiais", %w[Nome Categoria], student.conditions.map { |c|
      [c.name, t_enum(c, :category)]
    })
  ...
  def render_student_info_card(pdf)
    ...
    pdf.bounding_box([pdf.bounds.left, pdf.cursor], width: pdf.bounds.width) do
      ...
      data = [
        ...
        ["Situação:", t_enum(student, :situation)],
        ["Sala:", student.classroom&.name ? "#{student.classroom.name} (#{student.classroom.grade} - #{student.classroom.course} - #{student.classroom.year.year})" : "-"],
